...
WORD GetSyscall( PVOID pModuleBase, PIMAGE_EXPORT_DIRECTORY pImageExportDirectory, DWORD FunctionSysHash )
{
    ...
    for (WORD cx = 0; cx < pImageExportDirectory->NumberOfNames; cx++)
    {
        ...
        if ( KHashString( pczFunctionName, KStringLengthA(pczFunctionName) ) == FunctionSysHash )
        {
            ...
            {
                __movsb(&wSystemCall, (pFunctionAddress + 4), 2);
                return wSystemCall;
            }
            ...
            if (*((PBYTE)pFunctionAddress) == 0xe9)
            {
                for (WORD idx = 1; idx <= 500; idx++)
                {
                    ...
                    {
                        __movsb(&wSystemCall, (pFunctionAddress + 4 + idx * DOWN), 2);
                        return wSystemCall;
                    ...
                    {
                        __movsb(&wSystemCall, (pFunctionAddress + 4 + idx * UP), 2);
                        return wSystemCall;
                    }
